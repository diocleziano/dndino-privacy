...
Invece di aprire i luoghi solo dalla lista, puoi farlo partendo direttamente da una mappa.
...
Quindi una mappa interattiva è, in sostanza, una mappa immagine arricchita da marker navigabili.
...
Questa è la modalità migliore quando vuoi usare la mappa come navigazione principale della scena.
...
Questa modalità è particolarmente utile quando vuoi:
...
Nella dashboard luoghi questa mappa viene riconosciuta con l’indicatore di **mappa di default**.
...
Questo permette anche una navigazione gerarchica tra mappe più grandi e mappe più locali.
...
In questo modo puoi usare, per esempio:

- una mappa della città per aprire taverne, mercati e quartieri
- una mappa del castello per aprire stanze e sale
- una mappa regionale per aprire villaggi e punti di interesse
...
- l’avventura contiene più mappe
- vuoi passare manualmente da una mappa a un’altra
- vuoi vedere una mappa diversa da quella predefinita del luogo corrente
...
È utile quando hai cambiato manualmente mappa o sei passato attraverso una navigazione tra livelli.

### Livello superiore

Se il luogo corrente ha un luogo padre che possiede una mappa interattiva, può comparire il pulsante `Livello superiore`.

Questo ti permette di risalire a una mappa più ampia, per esempio:

- dalla stanza al castello
- dal quartiere alla città
- dal villaggio alla regione
...
Lo zoom viene limitato a un intervallo controllato, così la navigazione resta stabile e leggibile.
...
Questa opzione è utile quando vuoi tenere la mappa più pulita oppure più esplicita.
...
Quando fai click sulla mappa, DnDino crea il marker in quella posizione e lo seleziona.
...
Questo flusso è molto utile perché evita di creare marker “vuoti” o scollegati per sbaglio.
...
Il selettore usa un elenco ad albero appiattito, quindi riesci a distinguere anche i livelli di profondità tra luogo padre e sottoluogo.
...
Le coordinate vengono sempre mantenute nell’intervallo corretto della mappa.
...
L’eliminazione rimuove il marker dalla mappa e aggiorna subito la lista dei marker disponibili.
...
Inoltre, quando selezioni un luogo dall’albero logico della dashboard, DnDino cerca di conservare il marker corrispondente selezionato anche nella mappa, quando possibile.
...
In questi casi i marker diventano il ponte tra il livello più ampio e il dettaglio del luogo specifico.
...
Questo apre il pannello grande della mappa, più comodo per:
...
Usate bene, le mappe interattive trasformano la navigazione dei luoghi da elenco testuale a vera esplorazione visiva della campagna.
